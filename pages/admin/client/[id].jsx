  ...
  async function saveAdminFields() {
    ...
    if (error) {
      ...
      if (client.wedding_date) {
        const eventTitle = `${client.person1_first_name} ${client.person1_last_name} & ${client.person2_first_name} ${client.person2_last_name} — ${client.venue}`
        const description = `Venue: ${client.venue}\nPackage: ${adminForm.package}\nAssigned To: ${adminForm.assigned_to}\n${client.person1_first_name}: ${client.person1_email} · ${client.person1_phone}\n${client.person2_first_name}: ${client.person2_email} · ${client.person2_phone}`
        const color = djColor(adminForm.assigned_to)
        ...
        if (client.calendar_event_id) {
          ...
          // Create new calendar event and save the ID back
          await calendarEvent({
            action: 'create',
          ...
          // Since no-cors blocks the response, we store by title lookup
          // The event is created — update client record to mark calendar as synced
          await supabase
            .from('clients')
    ...
    <>
      ...
      <div className="min-h-screen bg-neutral-950 text-neutral-100">
        ...
        <main className="max-w-7xl mx-auto px-4 py-4 md:py-10">
          ...
          {/* Tab navigation */}
          <div className="flex gap-1 mb-4 border-b border-neutral-800 pb-0 overflow-x-auto scrollbar-none -mx-4 px-4">
            {TABS.map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-3 py-2 text-xs md:text-sm font-semibold capitalize transition-all border-b-2 -mb-px whitespace-nowrap ${
                  activeTab === tab
              ...
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
                ...
                {client.package === 'All-Inclusive Partner' ? (
                  <div className="bg-emerald-400/10 border border-emerald-400/20 rounded-xl p-4 text-center">
                    <p className="text-emerald-400 font-bold">✓ All-Inclusive Package</p>
                    <p className="text-xs text-neutral-400 mt-1">Your venue has covered the cost of DJ services. No payment required from you!</p>
                  </div>
                ) : (
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between py-2 border-b border-neutral-800">
